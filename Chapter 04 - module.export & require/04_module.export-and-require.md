...
-   A module is an encapsulated units of code that can be reused across different parts of an application.
...
-   Writing all the code in a single file makes it messy.
...
greet("Dhamareshwar"); // Output: Hello, Dhamareshwar!
```
...
-   For better organization, modules with related functionalities can be placed inside a folder, and an index file can be used to specify exports, which in turn becomes a module.
-   i.e., in the below case `add.js` and `subtract.js` kept together to form a `math` module that provides utilities for addition and substraction.
...
| Strict mode | No                        | Yes                |
| await       | not allowed at top level  | supports top-level |
